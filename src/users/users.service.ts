...
import { permissionsForRoleDef, userHasPermission } from '../auth/permissions';
...
export class UsersService {
  ...
  async update(actor: ActorCtx, id: string, dto: UpdateAdminUserDto) {
    ...
    if (
      current.roleDef.slug === 'superadmin' &&
      nextRole.slug !== 'superadmin'
    ) {
  ...
  async remove(actor: ActorCtx, id: string) {
    ...
    const current = await this.byId(id);
    if (
      !this.roles.canAssignRole(
        actor,
        permissionsForRoleDef(current.roleDef),
        current.roleDef.slug,
      )
    ) {
